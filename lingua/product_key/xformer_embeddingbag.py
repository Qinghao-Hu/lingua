...
class CustomEmbeddingBagFunction(torch.autograd.Function):
    ...
    def forward(ctx, indices, weight, per_sample_weights, mode="sum", backward_chunks=1):
        ...
        return F.embedding_bag(indices, weight, per_sample_weights=per_sample_weights, mode="sum")  # (bs * MemP, v_dim // MemP)
...
def embedding_bag_triton(indices: torch.Tensor, weight: torch.Tensor, per_sample_weights: torch.Tensor) -> torch.Tensor:
...
if __name__ == "__main__":
    ...
    # torch.set_default_dtype(torch.bfloat16) # TODO: Check this
    ...
    for op, op_name in [
        ...
        # (torch.compile(F.embedding_bag), "F.embedding_bag + torch.compile"),
...
# srun -J test -N 1 --gpus-per-node 8 --exclusive python lingua/product_key/xformer_embeddingbag.py
