...
class xFormerEmbeddingBag(nn.Module):
    ...
    def forward(self, indices, scores):
        ...
        output = xformers_embedding_bag(indices, weight, per_sample_weights=scores, mode="sum")
        return output
...
class ColwiseEmbeddingBag(ParallelStyle):
    ...
    def _prepare_output_fn(output_layouts, use_local_output, mod, outputs, device_mesh):
        outputs = DTensor.from_local(outputs, device_mesh, (Shard(-1),), run_check=False)
    ...
    def _apply(self, module: nn.Module, device_mesh: DeviceMesh) -> nn.Module:
        if isinstance(module, xFormerEmbeddingBag):
            ...
            raise NotImplementedError("ColwiseEmbeddingBag currently only support nn.EmbeddingBag!")
